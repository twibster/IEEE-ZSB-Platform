...
def get_all(mapper: Mapper, db: Session = next(get_db())) -> list[Mapper]:
...
def get_by_email(email: str, db: Session = next(get_db())) -> User | None:
    return db.query(User).filter_by(email=email).first()


def get_by_username(username: str, db: Session = next(get_db())) -> User | None:
    return db.query(User).filter_by(username=username).first()


def update_by_id(mapper: Mapper, id: int,
                 modified: dict, db: Session = next(get_db())) -> tuple[int, str]:
...
def delete_by_id(mapper: Mapper,
                 id: int, db: Session = next(get_db())) -> tuple[int, str]:
    instance = get_by_id(mapper, id)
    if instance:
        db.delete(instance)
        db.commit()
